...
public sealed class OpenAiLlmClient(IHttpClientFactory httpClientFactory, IOptions<AtlasAiOptions> options) : ILlmClient
{
    ...
    public async Task<string> CompleteAsync(IReadOnlyList<AtlasAiMessage> messages, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (response.StatusCode == HttpStatusCode.TooManyRequests)
                return "Limite de requisições atingido na OpenAI. " + await new FallbackLlmClient().CompleteAsync(messages, cancellationToken);
    ...
    private async Task StreamAsync(ChannelWriter<string> writer, IReadOnlyList<AtlasAiMessage> messages, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (response.StatusCode == HttpStatusCode.TooManyRequests)
            {
                await writer.WriteAsync("Limite de requisições atingido na OpenAI. Vou responder com base nos dados do CRM.\n\n", cancellationToken);
                await StreamFallbackAsync(writer, messages, cancellationToken);
                return;
            }
