    ...
    : IUserPreferencesService
{
    public async Task<UserPreferencesDto> GetAsync(long userId,CancellationToken ct=default)
    {
        ...
        var item=(await preferences.ListAsync(ct)).FirstOrDefault(x=>x.UserId==userId);
    ...
    public async Task<UserPreferencesDto> SaveAsync(long userId,UserPreferencesDto dto,CancellationToken ct=default)
    {
        ...
        var item=(await preferences.ListAsync(ct)).FirstOrDefault(x=>x.UserId==userId);
        if(item is null){item=new(){UserId=userId};await preferences.AddAsync(item,ct);}else preferences.Update(item);
...
public sealed class IdentityService(IRepository<Usuario> repository) : IIdentityService
{
    public async Task<CurrentUserDto> ResolveAsync(string auth0Sub,string? email,string? name,CancellationToken ct=default)
    {
        ...
        var users=await repository.ListAsync(ct);
        var user=users.FirstOrDefault(x=>x.Auth0Sub==auth0Sub)
            ?? users.FirstOrDefault(x=>!string.IsNullOrWhiteSpace(email)&&x.Email.Equals(email,StringComparison.OrdinalIgnoreCase));
        if(user is null)
        {
            ...
            while(users.Any(x=>x.Username.Equals(username,StringComparison.OrdinalIgnoreCase)))username=$"{prefix}{suffix++}";
